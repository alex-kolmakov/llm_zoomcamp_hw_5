...
def elasticsearch(
    ...
    connection_string = kwargs.get('connection_string', 'http://elasticsearch-1:9200')
    index_name_prefix = kwargs.get('index_name', 'documents')
    current_time = datetime.now().strftime("%Y%m%d_%M%S")
    index_name = f"{index_name_prefix}_{current_time}"

    set_global_variable('entropic_omega', 'index_name', index_name)
    ...
    print(f'Last document {document["document_id"]}')
